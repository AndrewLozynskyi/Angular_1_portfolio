/*
 * @author ohmed, oleg
 * Profile 'Contacts' tab controller
*/

angular.module( 'Profile.module' )

.controller( 'profileContacts.controller', [ '$scope', '$rootScope', 'contacts.service', 'users.service', '$mdDialog', '$cookies', 'profileContacts.service', function ( $scope, $rootScope, contactsService, usersService, $mdDialog, $cookies, contactsProfileService ) {
    ...
    angular.element( document )
        .find('.profile-card')
        .addClass('add-profile-shadow-position')

    angular.element( document )
        .find('#profile-card')
        .addClass('add-profile-shadow')
        .removeClass('hide-profile-shadow')

    angular.element( document )
        .find('.app-content')
        .addClass('add-profile-shadow-scroll')
    ...
    $scope.loadMoreCourses = function ( type, groupId ) {
        ...
        .then( function ( response ) {
            ...
            $scope.$watch ( function () {
                ...
                angular.element( document )
                    .find('.profile-card').removeClass('add-profile-shadow-position')

                angular.element( document )
                    .find('#profile-card').removeClass('add-profile-shadow').addClass('hide-profile-shadow')

                angular.element( document )
                .find('.app-content').removeClass('add-profile-shadow-scroll')
    ...
    $scope.initializeResultList = function () {
        ...
        contactsService.getList( userId, 'current', false, false, 0, 20, $this.sortattr, $this.sortdir )
        .then( function ( data ) {
